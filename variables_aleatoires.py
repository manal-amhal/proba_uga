...
# Moments théoriques de la loi de Bernoulli
esperance_X_k = p
variance_X_k = p * (1 - p)
...
# Moments théoriques de la loi Binomiale
...
# Calcul des probabilités ponctuelles P(S_n = k) via .pmf
...
# .pmf signifie probability mass function (fonction de masse de probabilité), 
# ce qui correspond à la loi de probabilité d'une variable discrète 
# le premier f signifie formatted (chaîne formatée); placé tout au début, 
# il active la fonctionnalité d'évaluation des variables entre {}
# le deuxième f signifie float (nombre à virgule); préciser qu'on applique 
# la règle des 6 décimales à un nombre décimal


# Fonction de répartition $F_S_n(k) = P(S_n <= k)$ via .cdf
...
# Représentation graphique de la fonction de répartition (étape discrète)
...
# pour le calcule de médiane soit on utilise .median ou .ppf qui signifie 
# percent point function (fonction quantile)
...
# np.arange(début, fin) : c'est une fonction de NumPy qui génère une liste ordonnée 
# de nombres (un tableau)
...
# figsize=(10, 5) : Spécifie la largeur et la hauteur de l'image en pouces 
# cela permet d'avoir un graphique bien lisible et étiré en largeur

# .x (axe horizontal) : les valeurs possibles de la variable aléatoire
...
x = np.arange(0, 51)
proba_binomiale = binom.pmf(x, n, p) 
proba_normal = norm.pdf(x, n * p, np.sqrt(n * p * (1 - p))) 
# pour calculer loi pour va continue .pdf(x,mu,ecart type)
...
# la fonction randint du module scipy.stats représente la loi uniforme discrète 
